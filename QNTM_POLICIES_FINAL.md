...
By creating an account or using QNTM, you agree to these Terms. If you don't agree, don't use the platform.
...
You must be 18 or older. By using QNTM you confirm this. You're responsible for complying with the financial research laws in your country.
...
- One free trial per customer. Trials are limited to one per person, per household, and per payment method. We reserve the right to decline a trial to any account where we reasonably believe a trial has already been used.
...
2. You'll receive an email notification each retry.
...
- **Account deletion ends the benefit.** If you delete your QNTM account, you forfeit Founding Member status. Re-registering with a new email does not restore it.
...
QNTM is provided "as is" and "as available." We don't guarantee the platform will always be available or that model scores will be accurate, current, or complete.
...
You and QNTM agree to resolve disputes only on an individual basis. Class actions, collective actions, and representative actions are waived to the extent permitted by law.
...
These Terms are governed by the laws of the State of California, without regard to conflict-of-laws principles. The Federal Arbitration Act governs the interpretation and enforcement of the arbitration provision.
...
**You give us:**
- Email address and password (passwords are bcrypt-hashed — we can't see them)
...
- We use HTTPS for all traffic.

## How Long We Keep Your Data

- **Active accounts:** kept while your account exists.
- **Deleted accounts:** personal data is removed within 30 days. Founding Member status is forfeited at deletion and cannot be restored — see Section 6 of the Terms.
...
QNTM does not currently publish a historical backtest. The only performance we show is the live Model Portfolio, which is marked daily on the model's rules-based signals from its inception date forward. It is a hypothetical illustration (see Model Portfolio above) and does not account for all real-world costs, taxes, or execution constraints. A live track record is short by nature, and past model performance is not a guarantee of future results. In June 2026 the scoring universe was expanded (from approximately 830 names to approximately 1,400) to add the Russell 1000 plus selected Russell 2000 small-caps. The Model Portfolio and its track record are not restated to reflect this change retroactively; the broader universe applies to selections from the expansion date forward, as positions turn over.
...
We do not currently use analytics, advertising, or third-party tracking cookies. If we add analytics in the future, we'll update this policy and require fresh consent.

## No Ad Tracking

We don't use advertising cookies. We don't track you across other websites. We don't share cookie data with advertisers.
...
The following items were intentionally left for your judgment. They are flagged here rather than buried in the draft so they get explicit attention.

1. **Arbitration provider.** Draft picks JAMS Streamlined Rules with venue in Orange County, California. Alternative is AAA Consumer Rules; AAA requires registration on the Consumer Clause Registry with annual maintenance fees, and 2025 case law (e.g., Merritt Island Woodwerx v. Space Coast Credit Union) showed that failure to register can waive arbitration. JAMS does not have an equivalent registry requirement.

2. **30-day arbitration opt-out** (Section 10 of Terms). Included as protection against California unconscionability arguments under the Concepcion / McGill line of cases. Confirm this is appropriate for our risk posture.

3. **Class action waiver** sits alongside the opt-out. California courts apply heightened scrutiny here. If you'd prefer to drop the waiver and rely on the individual-arbitration default, the Section 10 language is easy to adjust.

4. **Liability cap.** Set at the greater of trailing 12 months of fees or $100. The $100 floor is included because Free users pay nothing — some courts will not enforce a cap that nets to zero. Confirm.

5. **Model Portfolio disclosure** (Investment Disclaimer). The product visibly tracks "active positions" with entries, exits, and dollar-weighted P&L. This is the SEC's classic "holding out as an adviser" surface area. Draft is conservative; you may want to add language directly in the in-app copy (the Model Portfolio page footer currently reads "Model portfolio is hypothetical. Equal weight per position. Does not account for slippage, taxes, or transaction costs. For informational purposes only.").
