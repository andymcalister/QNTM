...
def factor_panel_html(r: dict, is_gem: bool = False, company_info: dict = None, card_id: str = None, rank: int = 0, suppress_wl_btn: bool = False) -> str:
    ...
    return (
        ...
        f'<label for="{cid}" onclick="(function(el){{'
        f'var w=el.closest(\'[data-card]\') || el.parentNode;'
        f'var d=w.querySelector(\'.qcard-detail\');'
        f'if(!d)return;'
        f'var open=d.style.display===\'block\';'
        f'document.querySelectorAll(\'.qcard-detail\').forEach(function(x){{x.style.display=\'none\';}});'
