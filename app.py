...
def page_screener():
    ...
    if st.session_state.scan_results is None:
        with st.spinner("Loading universe scores..."):
            # Always load static scores instantly first so the UI is responsive.
            # run_full_scan will serve from Supabase cache if fresh (milliseconds),
            # or fall back to static fundamentals (seconds, not minutes).
            # Live yfinance price fetching is intentionally skipped here —
            # data_refresh.py handles that nightly in the background.
