...
def page_screener():
    ...
    if st.session_state.scan_results is None:
        with st.spinner("Loading universe scores..."):
            # run_full_scan now reads from Supabase cache if fresh,
            # or falls back to static scoring. fetch_macro_overlay
            # now defaults to live RSS feeds.
            raw   = run_full_scan(use_live_prices=True)
...
def page_gems():
    ...
    if st.session_state.scan_results is None:
        st.session_state.scan_results = run_full_scan(use_live_prices=True)
